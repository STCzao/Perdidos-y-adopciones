...
export const validateTelefono = (value) => {
  ...
  if (!TELEFONO_RE.test(v))
    return `El teléfono debe contener entre ${RULES.TELEFONO_MIN} y ${RULES.TELEFONO_MAX} dígitos numéricos`;
  return null;
};
...
export const validateRegisterForm = ({ nombre, telefono, correo, password }) => {
  const errors = {};
